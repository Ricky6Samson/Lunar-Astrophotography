 **Lunar Astrophotography Project**
 
 *Observational Astronomy Certificate – St. Joseph’s University, Bengaluru*  
 *Under the guidance of **Dr. Ajay Saxena**, Former Dean – Indian Institute of Astrophysics (IIA), Bengaluru*

---

🔭 **About the Project**  

Captured, processed, and analyzed high-resolution images of the Moon using a **12-inch Ritchey–Chrétien Cassegrain Telescope** and smartphone sensors. This project focused on both **observational astronomy & computational image processing** to study lunar craters, maria, rilles, and impact basins.

---

 **Equipment & Capture Setup**
 
• 12-inch RC Cassegrain Telescope  
• Smartphone cameras (Realme X7 Max, Samsung Galaxy M32) using telescope mount  
• Moon filter + 44 mm eyepiece  
• Remote shutter trigger for vibration-free imaging  
• **Location:** St. Joseph’s University Observatory, Bengaluru (~900m altitude)

 *Observation Period:* 23–27 March 2024  
 *Phases Captured:* Waxing Gibbous → Full Moon → Waning Gibbous  
 *Camera Settings:* ISO 100 | 1/400s | f/11  

---

 **Processing Workflow**
 
| Software Used | Purpose |
|--------------|---------|
| **PIPP** | Frame extraction, alignment, video preprocessing |
| **AutoStakkert / Astrosurface** | Frame selection, stacking, pixel alignment |
| **RegiStax** | Wavelet sharpening, deconvolution, noise reduction |
| **Canva** | Overlay + Presentation |

 *Techniques Applied:* 
 
Wavelet Sharpening • Wiener Deconvolution • Gamma & Curves • Histogram Tuning • Noise Filtering

---

 **Lunar Features Identified**
 
 Tycho Crater  
 Mare Tranquillitatis *(Apollo 11 Landing Site)*  
 Copernicus Crater  
 Aristarchus Plateau + Schröter’s Valley  
 Mare Imbrium • Serenitatis • Crisium • Humorum, etc.  
 *Reference: NASA Daily Moon Guide*

---
 **Skills Gained**
 
✔ Telescope setup & observational planning  
✔ Smartphone astrophotography optimization  
✔ Planetary imaging, stacking, noise reduction  
✔ Wavelet-based sharpening & deconvolution  
✔ Geological interpretation of lunar features  

---

 **Project Team**
 
Raj Nandini Singh • Ricky Samson • Shahithya Vel • Chinmay Kulkarni

---

 **Conclusion**  
This hands-on astrophotography project transformed raw lunar data into meaningful scientific insights. By combining telescope operation with computational image processing, we explored the Moon’s geology one pixel at a time. 🚀🌕

